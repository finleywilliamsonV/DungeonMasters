package  {
	...
	public class UI_NavPanel extends MovieClip {
		...
		public function UI_NavPanel(mg:MasterGrid) {
			_masterGrid = mg;
		...
		public function onEnter(e:Event) : void {
			...
			if (_button is NavPanel_Up) {
				_masterGrid.y += _pixelWidth/4;
			} else if (_button is NavPanel_Down) {
				_masterGrid.y -= _pixelWidth/4;
			} else if (_button is NavPanel_Left) {
				_masterGrid.x += _pixelWidth/4;
			} else if (_button is NavPanel_Right) {
				_masterGrid.x -= _pixelWidth/4;
			}
		}
